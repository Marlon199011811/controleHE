 ...
 * Dashboard Interativo de Horas Extras
 * ─────────────────────────────────────────────────────────────
 * Lê planilha Excel (.xlsx) com múltiplas abas (uma por dia),
 * normaliza colunas variáveis, converte HH:MM → decimal e
 * alimenta todos os gráficos, tabela e alertas.
 */

/* ═══════════════════════════════════════════════════════════
   ESTADO GLOBAL
═══════════════════════════════════════════════════════════ */
let allData      = [];   // array de todos os registros normalizados
let filteredData = [];   // subset ativo (filtros de período/data)
let tableRows    = [];   // subset adicional (busca + equipe)
let activeDates  = new Set(); // abas/datas selecionadas nas chips

// Instâncias Chart.js (destruídas e recriadas em cada refresh)
const charts = {};
...
/**
 * Converte string "HH:MM" ou "-HH:MM" para número decimal.
 * Retorna 0 em caso de inválido.
 */
...
/** Soma array de decimais e retorna HH:MM */
...
/**
 * Mapeia cabeçalho variável → chave normalizada.
 * As abas 09-06 a 12-06 não têm colunas Cargo/Equipe;
 * a partir de 13-06 elas aparecem.
 */
...
/**
 * Parseia um workbook SheetJS e retorna array de registros normalizados.
 */
function parseWorkbook(wb) {
  ...
  wb.SheetNames.forEach(sheetName => {
    ...
    // Linha 3 (índice 3) = cabeçalho
    if (raw.length < 5) return;
    const headerRow = raw[3];
    const map = buildColMap(headerRow);
    if (map.nome === undefined) return; // aba sem estrutura esperada

    // Dados a partir da linha 4 (índice 4)
    for (let i = 4; i < raw.length; i++) {
      const row = raw[i];
      const nome = String(row[map.nome] || '').trim();
      // Pular linha vazia ou linha de TOTAIS
...
function initDashboard() {
  ...
  // Construir chips de data
  buildDateChips();

  // Selecionar tudo inicialmente
  activeDates = new Set(allData.map(r => r.data));
  refreshAll();

  // Preencher selects de equipe e data na tabela
  populateFilters();
}
...
function updateKPIs() {
  ...
  // Qtd colaboradores únicos com HE
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: { label: ctx => ' ' + dec2hhmm(ctx.raw) },
        },
      },
    },
  });
  ...
  charts.pizza = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...CHART_DEFAULTS.plugins.tooltip,
          callbacks: { label: ctx => ' ' + ctx.label + ': ' + dec2hhmm(ctx.raw) },
        },
      },
    },
  });
...
function renderTable() {
  ...
  page.forEach(r => {
    ...
    let cls = 'row-green';
    // Acima de 10h acumulado no período para esse nome
...
function updateAlerts() {
  ...
  const heByColab = {};
  filteredData.forEach(r => {
    heByColab[r.nome] = (heByColab[r.nome] || 0) + r.totalHE;
  });
  const accList = Object.entries(heByColab)
    .filter(([, v]) => v > 10)
    .sort((a, b) => b[1] - a[1]);
  ...
  // 2. Mais de 2h em algum dia
  const dailyHigh = filteredData.filter(r => r.totalHE > 2)
...
/* ═══════════════════════════════════════════════════════════
   AUTO-LOAD: se o arquivo Excel estiver no mesmo diretório
═══════════════════════════════════════════════════════════ */
(async function autoLoad() {
  try {
    ...
    // silently ignore — user will upload manually
  } finally {
    showLoading(false);
  }
})();
